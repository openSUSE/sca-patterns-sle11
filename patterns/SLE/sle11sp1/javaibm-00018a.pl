...
# Title:       IBM Java 1.4 Security Advisory SUSE-SA:2011:024
...
@PATTERN_RESULTS = (
	...
	"META_LINK_Security=http://www.novell.com/linux/security/advisories/2011_24_ibmjava.html"
);
...
SDP::Core::processOptions();
	...
	if ( SDP::SUSE::compareKernel(SLE11SP1) >= 0 && SDP::SUSE::compareKernel(SLE11SP2) < 0 ) {
		...
		$FIXED_VERSION = '1.4.2_sr13.9-0.4.1';
		...
		$FIXED_VERSION = '1.4.2_sr13.9-0.7.1';
		SDP::SUSE::securitySeverityPackageCheck($CHECKING, $SEVERITY, $TYPE, \@PKGS_TO_CHECK, $FIXED_VERSION);
		...
		if ( $HOST_INFO{'architecture'} eq 'x86_64' ) {
			$FIXED_VERSION = '1.4.2_sr13.9-0.7';
		} else {
			$FIXED_VERSION = '1.4.2_sr13.9-0.6';
		}
		SDP::SUSE::securitySeverityPackageCheck($CHECKING, $SEVERITY, $TYPE, \@PKGS_TO_CHECK, $FIXED_VERSION);
	} else {
