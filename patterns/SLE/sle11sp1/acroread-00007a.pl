...
# Title:       Acrobat Reader Security Advisory SUSE-SA:2010:037
...
@PATTERN_RESULTS = (
	...
	"META_LINK_Security=http://www.novell.com/linux/security/advisories/2010_37_acroread.html"
);
...
SDP::Core::processOptions();
	...
	my @PKGS_TO_CHECK = ();
	my $FIXED_VERSION = '';
	if ( SDP::SUSE::compareKernel(SLE11GA) >= 0 && SDP::SUSE::compareKernel(SLE11SP2) < 0 ) { # excluded acroread_ja due to different version, it's easier to exclude it than change the code
		@PKGS_TO_CHECK       = qw(acroread acroread-cmaps acroread-fonts-ja acroread-fonts-ko acroread-fonts-zh_CN acroread-fonts-zh_TW);
		$FIXED_VERSION       = '9.3.4-0.6.1';
